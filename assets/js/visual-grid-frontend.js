...
(function () {
  ...
  function vgLog() {
    if (
      window.visual_grid_settings &&
      window.visual_grid_settings.console_output
    ) {
    ...
    switch (style) {
      ...
      case "dotted":
        return (
          ...
          " 0px, " +
          color +
          " 2px, transparent 2px, transparent 6px)"
        );
      case "solid":
      default:
        return color;
  ...
  function shouldPreventOverlap(pos, side, preventOverlap) {
    ...
    switch (side) {
      case "top":
        key = Math.round(pos.top);
        break;
      case "bottom":
        key = Math.round(pos.bottom);
        break;
      case "left":
        key = Math.round(pos.left);
        break;
      case "right":
        key = Math.round(pos.right);
        break;
      default:
        return false;
    }
   ...
   */
  function splitLineByExclusions(
    lineStart,
    lineEnd,
    exclusionRects,
    isHorizontal,
    ...
    relevantExclusions.sort(function (a, b) {
    ...
    for (var i = 0; i < relevantExclusions.length; i++) {
      ...
      if (excl.start > currentStart) {
        segments.push({
          start: currentStart,
          end: Math.min(excl.start, lineEnd),
        });
      }
  ...
  function createLineElement(side, config, pos, docHeight, docWidth) {
    ...
    if (side === "top" || side === "bottom") {
      ...
      line.style.background = getBackgroundStyle(
        config.style,
        color,
        "horizontal",
      );
  ...
  function processElement(element, container, docHeight, docWidth) {
    ...
    if (
      visualGrid.overlaySelectors &&
      visualGrid.overlaySelectors.trim() !== ""
    ) {
      var overlayList = visualGrid.overlaySelectors
        .split(",")
        .map(function (s) {
          return s.trim();
        })
      ...
      for (var i = 0; i < overlayList.length; i++) {
        try {
          ...
          vgWarn(
            'Visual Grid: Invalid overlay selector "' + overlayList[i] + '"',
            e,
          );
        }
      }
    ...
    if (visualGrid.linked) {
      ...
      if (visualGrid.bottom && visualGrid.bottom.enabled)
        sides.bottom = visualGrid.bottom;
      if (visualGrid.left && visualGrid.left.enabled)
        sides.left = visualGrid.left;
      if (visualGrid.right && visualGrid.right.enabled)
        sides.right = visualGrid.right;
    }
    ...
    for (var side in sides) {
      if (sides.hasOwnProperty(side)) {
        ...
        var segments = splitLineByExclusions(
          lineStart,
          lineEnd,
          exclusionRects,
          isHorizontal,
  ...
  function init() {
    ...
    // Remove existing container FIRST to avoid chicken-and-egg height inflation.
    // Old grid lines are absolutely positioned and can stretch scrollHeight,
    // so they must be gone before we measure the document.
    var existing = document.getElementById("visual-grid-container");
    if (existing) {
      existing.parentNode.removeChild(existing);
    }

    // Force a synchronous reflow so the browser recalculates layout
    // without the old grid container inflating the dimensions.
    ...
    // Prefer body-based measurements because the grid container now lives
    // on <html>, so documentElement.scrollHeight could still be influenced
    // by it in edge cases. body.scrollHeight is clean after removal above.
    ...
    // Append to <html> instead of <body>.
    // Absolutely-positioned children of <html> do NOT inflate
    // body.scrollHeight, which prevents the height-feedback loop.
    // Combined with CSS "contain: size layout" on the container,
    // this is robust across all browsers.
  ...
  // Use double-rAF instead of setTimeout to ensure the browser has
  // finished scroll-restoration before we measure document height.
  // This prevents the race condition where scrollY from the previous
  // page inflates scrollHeight on navigation via link.
